...
def convert_file(filename, file_path, aac_file, flac_file, q):
    ...
    logger.info(colored(f"Conversion de {os.path.basename(filename)} terminée.", 'green'))
    q.put(1)
            ...
            for file in files:
                if file.endswith('.wav'):
                    ...
                    if not (os.path.exists(aac_file) and os.path.exists(flac_file)):
                        ...
                        logger.info(colored(f"Conversion de {os.path.basename(filename)}...", 'blue'))
                        thread = threading.Thread(target=convert_file, args=(filename, file_path, aac_file, flac_file, q))
                        thread.start()
                        threads.append(thread)
                    else:
                        logger.info(colored(f"Les fichiers de sortie pour {os.path.basename(filename)} existent déjà, aucune recompression nécessaire.", 'yellow'))
                        q.put(1)
